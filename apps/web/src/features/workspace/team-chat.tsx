  ...
  return (
    <section className="ck-panel nerv-chat" aria-labelledby="team-chat-title">
      <header className="nerv-section-header">
        ...
        <span className="ck-tag">as {workspace.viewer?.name ?? "…"}</span>
      </header>
        ...
        {workspace.messages.length === 0 ? (
          ...
          workspace.messages.map((message) => (
            <article key={message.id} className="nerv-message">
              <header>
                <strong>{workspace.profileName(message.profileId)}</strong>
      ...
      >
        ...
        <textarea
          ...
          rows={2}
          placeholder={`Message the team as ${workspace.viewer?.name ?? "..."}…`}
        />
        <button
          type="submit"
