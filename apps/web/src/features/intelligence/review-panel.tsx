...
import type { Proposal, Review, Snapshot, Task } from "@/contracts/schemas";
...
function useClock(timeZone: string) {
  return new Intl.DateTimeFormat("en", { timeStyle: "short", timeZone });
}
      ...
      {cited.map((task) => (
        <li key={task.id}>
          ...
          <p className="ck-muted">
            {task.githubLabels.length > 0 ? `Labels ${task.githubLabels.join(", ")} · ` : ""}
            updated {task.githubUpdatedAt ?? "unknown"}
...
function ReviewCard({
  ...
  const clock = useClock(workspace.viewer.timeZone);
    ...
    <div className="nerv-review">
      <header className="nerv-review-head">
        <div>
          <h3>Risk review</h3>
          <p className="ck-muted">
            {review.source === "model" ? "Risk specialist" : "Rule-based reading"} ·{" "}
            {clock.format(new Date(review.createdAt))} · charter {review.charterStatus}
          </p>
        </div>
      ...
      <section className="nerv-review-block">
        ...
        <ul className="nerv-limits">
          {review.observations.map((item) => (
            <li key={item}>
              <span className="nerv-kind">Observed</span> {item}
            </li>
          ))}
          {review.inferences.map((item) => (
            <li key={item}>
              <span className="nerv-kind">Inferred</span> {item}
            </li>
          ))}
...
function ProposalCard({
  ...
  const clock = useClock(workspace.viewer.timeZone);
  const isLead = workspace.viewer.accessRole === "lead";
  const busy = intelligence.busy !== null;
    ...
    <div className="nerv-review">
      ...
      <section className="nerv-review-block">
        ...
        <div className="nerv-issue">
          <div className="nerv-issue-head">
            ...
            <span className="ck-muted">{intelligence.snapshot?.repo ?? "configured repo"}</span>
          </div>
        ...
        {cited.length === 0 ? (
          ...
          <div className="nerv-badges">
            ...
            <span className="nerv-badge">
              snapshot {intelligence.snapshot?.mode ?? "unknown"}
            </span>
          </div>
        )}
      </section>
      ...
      <p className="ck-muted nerv-note">
        {isLead
          ...
          : `Only the lead can approve. You are acting as ${workspace.viewer.displayName}.`}
      </p>
    </div>
  );
}
...
function AppliedCard({
  ...
  const card = workspace.tasks.find((task) => task.githubIssueNumber === result?.number);
    ...
    <div className="nerv-review">
      <header className="nerv-review-head">
        <div>
          <h3>Issue created</h3>
          <p className="ck-muted">
            Approved by {workspace.memberName(proposal.approvedByMemberId)}
            {proposal.approvedAt === null
              ? ""
              : ` · ${clock.format(new Date(proposal.approvedAt))}`}
          </p>
        </div>
      ...
      <div className="nerv-applied">
        ...
        <p className="ck-muted">
          Confirmed by GitHub: number, URL and repository all match the approved text.
        </p>
      </div>
...
function UncertainCard({
  ...
  const repo = intelligence.snapshot?.repo;
  return (
    <div className="nerv-review">
      <header className="nerv-review-head">
        <div>
      ...
      <section className="nerv-review-block">
        ...
        <ol className="nerv-steps">
          ...
          <li>
            Not finding it in a partial read does not prove it is absent, so the state stays
            uncertain rather than flipping to failed.
          </li>
      ...
      <section className="nerv-review-block">
        ...
        <p className="nerv-marker">{proposal.marker}</p>
      </section>
      ...
      <p className="ck-muted nerv-note">
        Approving is disabled for this proposal so a retry cannot create a duplicate.
      </p>
    </div>
  );
}
        ...
        <div>
          ...
          <p className="ck-muted">
            {rejected
              ...
              : (proposal.error?.message ?? "GitHub refused the request.")}
          </p>
        </div>
  ...
  const body = (() => {
    ...
    if (proposal !== null) {
      if (proposal.status === "applied") {
        return (
          <AppliedCard proposal={proposal} intelligence={intelligence} workspace={workspace} />
        );
      }
      if (proposal.status === "uncertain" || proposal.status === "executing") {
        return <UncertainCard proposal={proposal} intelligence={intelligence} />;
      }
      ...
      {intelligence.notice === null ? null : (
        <p className="ck-error" role="alert">
          ...
          <button
            ...
            onClick={intelligence.dismissNotice}
          >
            Dismiss
          </button>
        </p>
